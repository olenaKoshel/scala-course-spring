...
object AdditionSpecification extends Properties("Addition"):
  ...
  property("left + right should be correctly evaluated") = forAll(genSmallNonNegativeNumber, genSmallNonNegativeNumber):
    (left: Number, right: Number) => addition(left, right) == left + right
...
object MultiplicationSpecification extends Properties("Multiplication"):
  ...
  property("left * right should be correctly evaluated") = forAll(genSmallNonNegativeNumber, genSmallNonNegativeNumber):
    (left: Number, right: Number) => multiplication(left, right) == left * right

end MultiplicationSpecification

object PowerSpecification extends Properties("Power"):

  property("base ^ p should be correctly evaluated") = forAll(genSmallNonNegativeNumber, genSmallNonNegativeNumber):
